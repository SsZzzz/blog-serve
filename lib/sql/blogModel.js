...
exports.getCommentList = (value) => {
  const sql = "SELECT comment,DATE_FORMAT(releaseTime,'%Y-%m-%d %h:%i %p') releaseTime,avator,username FROM blog_comment a LEFT JOIN blog_user b ON a.userid=b.id WHERE articleid=? ORDER BY a.releaseTime DESC";
  return db.query(sql, value);
}