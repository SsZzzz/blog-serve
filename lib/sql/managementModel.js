...
exports.updateArticle = (value) => {
  const sql = "UPDATE blog_article SET title=?,titleImage=?,content=? WHERE id=?";
  return db.query(sql, value);
}
...
exports.delLabel = (value) => {
  const sql = "DELETE FROM blog_label WHERE blogid=?";
  return db.query(sql, value);
}
exports.getSummary = () => {
  //分别是用户总数,总访问量和日访问量
...
exports.delArticle = (value) => {
  const sql = "UPDATE blog_article SET isDelete=1 where id=?";
  return db.query(sql, value);
}